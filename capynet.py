    ...
    def run(self):
        ...
        def displaypackets(packet):
            ...
            if packet.haslayer(IP):
                if packet.haslayer(IP):
                    ...
                    if packet.haslayer(Ether):
                        ...
                        if destination_mac == "ff:ff:ff:ff:ff:ff":
                            packet_protocol="Broadcast"
                    if packet.haslayer(TCP) and (packet.sport == 443 or packet.dport == 443):
                        packet_protocol = "TLSv1.2"  # Recognizing TLSv1.2
                    elif packet.haslayer(TCP) and (packet.sport == 80 or packet.dport == 80 or packet.dport == 8060 or packet.sport == 8060):
                        packet_protocol = "HTTP"
                    elif packet.haslayer(UDP) and (packet.dport == 53 or packet.sport == 53):
                        packet_protocol = "DNS"
                    elif packet.haslayer(UDP) and (packet.dport == 1900 or packet.sport == 1900):
                        packet_protocol = "SSDP"  # Recognizing SSDP
                    else:
                        packet_protocol = protocol_number_dictionary[protocol_number]
                        if packet_protocol is None or not packet_protocol:
                ...
                # Recognize protocols (e.g., TLSv1.2, HTTP, DNS) for IPv6
                if packet.haslayer(TCP) and (packet.sport == 443 or packet.dport == 443):
                    packet_protocol = "TLSv1.2"
                elif packet.haslayer(TCP) and (packet.sport == 80 or packet.dport == 80 or packet.dport == 8060 or packet.sport == 8060):
                    packet_protocol = "HTTP"
                elif packet.haslayer(UDP) and (packet.dport == 53 or packet.sport == 53):
                    packet_protocol = "DNS"
...
def on_click_packet(e):
    ...
    #content for t3
        elif packets_list_storage[val_No].haslayer(TCP):
